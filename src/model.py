...
class GATNet(torch.nn.Module):
    def __init__(self):
        ...
        self.conv_layer3 = GATv2Conv(
            in_channels=64 * 8, out_channels=64, heads=8, concat=True, edge_dim=7
        )
        self.conv_layer4 = GATv2Conv(
            in_channels=64 * 8, out_channels=32, heads=4, concat=True, edge_dim=7
        )
        self.conv_layer5 = GATv2Conv(
            in_channels=32 * 4, out_channels=1, heads=1, concat=False, edge_dim=7
        )
    ...
    def forward(self, data):
        ...
        X, attention_weights = self.conv_layer5(X, edge_index, edge_attr, return_attention_weights=True)
